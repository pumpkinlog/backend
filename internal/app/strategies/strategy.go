...
func (s *BaseStrategy) filterPresencesByWindow(presences []*domain.Presence, window domain.TimeWindow) []*domain.Presence {
	var filtered []*domain.Presence
	for _, p := range presences {
		if p.Date.After(window.Start) && p.Date.Before(window.End) {
